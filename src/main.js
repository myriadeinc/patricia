
// const blockReferenceService = require('./block.reference.js');
const WebSocketRPCServer = require('rpc-websockets').Server
const cache = require('./util/cache.js')
const mq = require('./util/mq.js')
const service = require('./stratum.service.js')

const exampleTemplate = {

		"blockhashing_blob": "0e0e8c89eb9606b44e67bdbc91e5f95c5ea49ba410b1c223ab897417b19a0f631b80cf5be43a4e00000000f608c45071e983a7fbcd9d1474bfa1505d4d808b2bfcd513f214a3820c41942d17",
		"blocktemplate_blob": "",
		"difficulty": 307042777507,
		// "difficulty": 77507,
		"difficulty_top64": 0,
		"expected_reward": 600450790000,
		"height": 2672960,
		"next_seed_hash": "",
		"prev_hash": "b44e67bdbc91e5f95c5ea49ba410b1c223ab897417b19a0f631b80cf5be43a4e",
		"reserved_offset": 130,
		"seed_hash": "348b6ead39ba7ebb7ab45bbada3e8e7c3c171e257671e5ac570867c953d5bcf9",
		"seed_height": 2672640,
		"status": "OK",
		"untrusted": false,
		"wide_difficulty": "0x477d2cf9a3"
	
}



async function main() {
  
const server = new WebSocketRPCServer({
  port: 9877,
  host: '0.0.0.0'
})
cache.init('localhost:6379')
await mq.init('amqp://guest:guest@localhost:5672/');
await cache.put('blocktemplate',exampleTemplate)

t = await cache.get('blocktemplate')
// console.log(t)
// server.register('verifyBlock', params => {
//   const {
//     blob, 
//     nonce, 
//     extraNonce, 
//     seed_hash, 
//     result
//   } = params[0]
//   return blockReferenceService.verifyBlock(blob, 
//     nonce, 
//     extraNonce, 
//     seed_hash, 
//     result)
// })

// server.register('checkDifficulty', params => {
//   const {
//     localDiff, globalDiff, block
//   } = params[0]
//   return blockReferenceService.checkDifficulty(localDiff, globalDiff, block)
// })

// newjob
// submitjob
server.register('newjob', service.NewJob)
server.register('submitjob', service.SubmitJob)



console.log(`Websocket server started on 9877`)
}



main().catch(err=>console.log(err));
