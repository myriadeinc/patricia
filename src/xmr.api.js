...
const MoneroService = {
    ...
    GetBlockTemplate: async (timeout = 1) => {
        payload = {
            "jsonrpc":"2.0",
            "id":"1",
        ...
        const result = await MoneroService.send_rpc(payload)
        if (result) {
            const save = await cache.put('blocktemplate', result, 0)
            return save            
        }
        delay(timeout * 3000)
        return MoneroService.GetBlockTemplate(timeout * 2)
    },
